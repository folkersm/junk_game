...
func activate(loc, code, type, source):
	...
	if code == "building added":
		print("debug: got inside clover building added block")
		var rando = rng.randi_range(0,20)
		if level >= rando:
			upgrade_an_adjacent()
		else:
			print("failed to upgrade %f %f" % [level, rando])
			
func upgrade_an_adjacent():
	print("modded adjacent")
	var adjacents = []
	for x in [-1,0,1]:
		for y in [-1,0,1]:
			if not (x==0 and y==0):
				...
				if sx >= 0 and sx < 5 and sy >= 0 and sy < 5:
					...
					if obj != null and obj.has_method("mod_level"):
						print(obj.get_class(), "debug printing class of object in clover adjacentcy check")
						adjacents.append(obj)
	print("clover debug", adjacents)
	if adjacents != []:
		print("clover at ", location, " has adjacents: ",adjacents)
		$AnimationPlayer.play("wiggle")
		var index = rng.randi_range(0,len(adjacents)-1)
		adjacents[index].mod_level(1)
	else:
		print("no adjacent objects to clover at ", location)
